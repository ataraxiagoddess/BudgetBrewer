...
interface ExpenseDao {
    ...
    @Query(
        "SELECT * FROM expenses WHERE categoryId IN (SELECT id FROM expense_categories WHERE budgetId = :budgetId)"
    )
    fun getExpensesForBudget(budgetId: String): Flow<List<Expense>>

    @Query(
        "SELECT SUM(amount) FROM expenses WHERE categoryId IN (SELECT id FROM expense_categories WHERE budgetId = :budgetId)"
    )
