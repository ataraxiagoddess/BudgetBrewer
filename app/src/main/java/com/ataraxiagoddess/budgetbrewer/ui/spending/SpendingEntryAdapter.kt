...
class SpendingEntryAdapter(
    ...
    inner class ViewHolder(private val binding: ItemTransactionBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(entry: SpendingEntry) {
            ...
            binding.ivTag.visibility =
                if (tagsEnabled && !entry.tag.isNullOrEmpty()) View.VISIBLE else View.GONE
            binding.ivNote.visibility =
                if (tagsEnabled && !entry.note.isNullOrEmpty()) View.VISIBLE else View.GONE
